...
## 1. Does UDAP Actually Become the Central Database?

Yes, with caveats. UDAP has the foundation: watermark-based sync engine, PostgreSQL with RLS, dbt semantic layer, an existing ingest API, and a production Data Middleware layer with YAML-based column mapping, value translation, PII redaction, and schema validation. But three data flows are missing today:

### What flows INTO UDAP now
...
| ADR | Case background for mediation context, agency-wide settlement trends | UDAP MCP tools or direct REST API |
| Triage | Historical classification patterns, agency-wide case distributions | UDAP MCP tools or direct REST API |
| OGC Trial Tool | Case history, investigation timeline, litigation context | UDAP MCP tools or direct REST API |
...
| **Pull from UDAP** | Agency-wide analytics, case context, settlement trends | UDAP MCP tools or REST API |
...
| **Push to UDAP** | Daily metrics, AI reliance scores, model drift, scheduling analytics | ADR → UDAP ingest API (daily push from ADR Azure Functions) |
...
| **Pull from UDAP** | Historical classification patterns, case distributions | UDAP MCP tools or REST API |
| **Push to ARC** | Classification results (rank, score, summary), correction events | Triage → ARC Integration API write-back endpoints |
| **Push to UDAP** | Daily metrics, correction flows, reliance scores, model drift | Triage → UDAP ingest API (daily push from Triage Azure Functions) |

### UDAP Analytics

| Direction | What | Path |
|-----------|------|------|
| **Pull from ARC** | Full charge lifecycle data (charges, allegations, staff, closures, mediation) | ARC Integration API feed endpoints → UDAP sync engine |
| **Pull from ADR** | Operational analytics (metrics, reliance, drift) | ADR pushes to UDAP ingest API |
| **Pull from Triage** | Classification analytics (metrics, corrections, drift) | Triage pushes to UDAP ingest API |
| **Serve to all** | Governed, RLS-enforced analytics views | UDAP MCP tools + REST API + direct PostgreSQL |
...
| **Pull from UDAP** | Case history, investigation timeline | UDAP MCP tools or REST API |
...
2. **Query UDAP** for any case data, analytics, or cross-system context it needs
3. **Call ARC Integration API write-back tools** through the hub if it needs to push data to ARC
4. **Push its own operational analytics to UDAP** via the ingest API if it generates data worth centralizing
...
Logical replication slot on PrEPA's PostgreSQL (pg_create_logical_replication_slot), publication for core tables (charge_inquiry, charging_party, respondent, charge_allegation, charge_assignment), Debezium connector deployment (Kubernetes or Container Apps), Azure Event Hub namespace provisioning (Kafka-enabled, 7-day retention, consumer group for UDAP middleware). Fallback documentation if ARC team cannot grant WAL access: Service Bus subscription + REST API feed endpoints.

### Missing Prompt: UDAP Middleware Reconciliation Engine
Twice-weekly comparison of UDAP analytics tables against IDR (SQL Server nightly snapshot) to detect missing or stale records. Row count comparison, sample checksum validation (SHA-256 on 1000 random rows by primary key), auto-backfill of missing records using existing sqlserver_*.yaml mappings. Alert if discrepancy exceeds 0.1% of total rows. New PostgreSQL table: middleware.reconciliation_log. Kubernetes CronJob: Tuesday + Friday at 03:00 UTC. The IDR stays alive as a safety net while the CDC pipeline proves itself.

### Missing Prompt: UDAP New Tables for ADR and Triage Data
UDAP needs new PostgreSQL tables and dbt models for ADR operational analytics and Triage classification data that is pushed via the ingest API.

### Missing Prompt: ADR → UDAP Analytics Push
ADR needs an Azure Function that pushes daily metrics, reliance scores, and drift data to UDAP's ingest API.

### Missing Prompt: Triage → UDAP Analytics Push
Same pattern: Triage pushes classification metrics, correction flows, and reliance data to UDAP.
...
## 4.5 The UDAP Data Middleware Is a First-Class Component
...
Sessions (ADR Redis-backed, UDAP Redis-backed), database connections (singleton + pooling), MCP servers (stateless request-scoped), blob storage (SAS tokens, no contention), Azure Table Storage writes (sequential, good partition design for non-case tables), Kubernetes HPA configured (UDAP 2-6 replicas), full-text search (GIN indexes acceptable), RLS overhead (negligible), concurrent request handling (gunicorn + Flask g context), no WebSocket dependencies, Azure Cognitive Search connection pooling, malware scanning (Event Grid scales), WORM blob writes (unique names).
...
Audit of the UDAP AI Assistant against the requirement for a full conversational AI assistant capable of multi-turn conversations, complex database queries, and visualization generation.
...
- **UDAP**: Parameterized queries everywhere (no SQL injection). Zero hardcoded secrets. Strong Entra ID + RLS. Non-root containers.
