...
**Date:** 2026-03-30
**Scope:** REST API layer over ARC backbone + MCP hub for ADR, OFS Triage, UDAP, and OGC Trial Tool
...
| Direction | ADR | OFS Triage | UDAP | OGC Trial Tool |
|-----------|-----|------------|------|----------------|
...
| **Pushes to UDAP** | Daily metrics, AI reliance, model drift, scheduling analytics | Daily metrics, correction flows, reliance scores | N/A (UDAP is the destination) | None |
| **Queries UDAP** | Agency-wide analytics, case context, settlement trends | Historical patterns, case distributions | N/A (serves queries to all) | Case history, investigation context |

### 2.1 What Each App Needs from ARC

| What the app needs from ARC | ADR | OFS Triage | UDAP | OGC Trial Tool |
|----------------------------|-----|------------|------|----------------|
...
| **UDAP** | MCP REST (5 endpoints), MCP JSON-RPC at `/mcp`, narrative search, document API | Dynamic tool catalog: 3 built-in tools plus N auto-generated from dbt models. Feature-flagged, protocol version 2025-03-26 | Entra ID for staff. MCP uses Entra M2M with Analytics.Read/Analytics.Write roles |
...
- UDAP bypasses the API layer entirely and pulls from the SQL Server database directly.
...
PrEPA already publishes case changes to Azure Service Bus (`db-change-topic`), but only the IMS internal ecosystem consumes it. None of the four apps subscribe. ADR has its own webhook-based event dispatch. OFS Triage is pull-only. UDAP does watermark-based SQL sync. OGC Trial Tool has nothing.
...
The MCP Hub Three-Application Integration document covers ADR, OFS Triage, and UDAP. After comparing it against the actual codebases, here is what holds up, what is missing, and where it conflicts with reality.
...
| UDAP has a dynamic tool catalog from dbt | Confirmed. Tools auto-generate from manifest.json on each dbt run |
| UDAP Row-Level Security is a blocker for hub integration | Confirmed. Session context must carry the caller's region claim or queries return silently wrong data |
...
| ADR/Triage use MCP.Read/MCP.Write; UDAP uses Analytics.Read/Analytics.Write | Confirmed. Intentional, not an oversight |
| Sequenced rollout (ADR first, then Triage, then UDAP) | Confirmed. ADR is most mature; UDAP has the RLS blocker that needs resolution first |
...
**The document never addresses how the hub gets data from ARC.** It describes hub-to-spoke connections but assumes ARC data just appears. ADR's ARCSyncImporter calls an endpoint that does not exist in the codebase we reviewed. UDAP reads directly from SQL Server. There is no plan for a proper ARC data path.
...
| UDAP tools are analytics, not case management | UDAP's analytics.charges table IS case data, pulled from the legacy IMS SQL Server via direct database access | Eventually UDAP should consume from the API layer instead of hitting SQL Server directly |
...
```
  ...
  Triage ──classifies──► ARC Integration        |
                          API              WAL (logical replication)
                           ...
                           ▼               Debezium → Event Hub
                         PrEPA                  |
                       (system of          UDAP Data Middleware
                        record)            (YAML translation,
                                  ...
                                  (analytics) (context) (litigation)
                                     ...
                                     MCP Hub queries UDAP
                                     for AI consumers

        IDR (SQL Server) ─── reconciliation (Tue + Fri) ──► UDAP Middleware
```

**UDAP is the central internal data repository.** It ingests case data from PrEPA in real-time via WAL/CDC (PostgreSQL logical replication → Debezium → Event Hub → Data Middleware). All data passes through the UDAP Data Middleware, which provides YAML-driven column translation, value mapping, PII redaction, and schema validation. The IDR (nightly SQL Server snapshot) serves as a twice-weekly reconciliation source. All analytics, cross-system queries, and AI-driven lookups go through UDAP's governed, RLS-enforced views.
...
#### Reference Data (consumed by UDAP feed and available to all apps)
...
Note: litigation data also flows into UDAP through the feed endpoints for
...
Note: Service Bus events are used here for cache invalidation and inter-app notifications. UDAP's primary data feed is the WAL/CDC pipeline (see Section 4.7), not the Service Bus.

### 4.7 WAL/CDC Data Pipeline

The primary path for getting case data from ARC into UDAP is PostgreSQL logical replication via WAL (Write-Ahead Log):
...
PrEPA PostgreSQL (9.x)
    ...
    │     │
    │     └── Logical replication slot: udap_cdc (read-only)
    │           │
    │           └── Debezium PostgreSQL connector
    │                 │
    │                 └── Azure Event Hub (Kafka protocol)
    │                       │
    │                       └── UDAP Data Middleware
    ...
    │                             │
    │                             └── UDAP PostgreSQL analytics tables
...
The IDR (Integrated Data Repository) is a nightly SQL Server snapshot of ARC data. It is currently UDAP's primary data source. Under the new architecture, it transitions to a twice-weekly reconciliation source:

| Phase | IDR Role | Frequency |
|-------|----------|-----------|
| Current | Primary data source for UDAP | Nightly |
...
The ARC Integration API registers as a spoke, but its role is different from the other spokes. It provides **write-back tools** (ADR and Triage pushing results to ARC) and a small number of **targeted read tools** (case pushes that need real-time data from ARC, not the UDAP data store).

**Targeted read tools (for real-time lookups that cannot wait for UDAP sync):**
...
Note: UDAP's data feed comes from the WAL/CDC pipeline (Section 4.7), not from the ARC Integration API. The ARC Integration API's role in the hub is write-back + targeted reads. Reference data endpoints are consumed by apps directly, not through the hub.
...
| ADR | 4 | 6 | 0 | 10 |
| OFS Triage | 5 | 3 | 0 | 8 |
| UDAP | 3 | 0 | N (dbt models) | 3+N |
| ARC Integration | 3 | 8 | 0 | 11 |
| OGC Trial Tool | 2 | 1 | 0 | 3 |
| **Total** | **17** | **18** | **N** | **35+N** |

Note: the ARC Integration API's read tool count dropped from 10 to 3 because most case query functionality is served by UDAP, not by live ARC lookups. The remaining 3 reads are targeted real-time lookups (mediation eligibility, charge metadata at upload, document metadata from ECM).
...
|-------------|-------------------|-------------------|
| Case analytics | UDAP analytics + reporting tools | ~6+N |
| Narrative search | UDAP narrative_search | ~2 |
...
UDAP data feed (separate path, not through hub):
...
| UDAP | No | client_id hashed | Not aligned | Add request_id, align tag |
...
| 5 | Parallel run: UDAP ingesting from both IDR (existing sqlserver_*.yaml) and WAL/CDC pipeline (new prepa_*.yaml). Compare data parity. Run reconciliation engine against both sources. |
| 6 | UDAP cut over to WAL/CDC as primary data source. IDR shifts to reconciliation-only (twice weekly, Tue + Fri). Validate sub-second CDC latency. |
...
### Phase 3: UDAP as Primary Query Spoke + OBO (Weeks 6-8)

Connect UDAP to the hub as the primary query target for AI consumers.

| Week | What Gets Done |
|------|---------------|
| 6-7 | OBO flow implementation so hub preserves caller region claims. UDAP team signs off on approach. Entra ID OBO configuration |
| 7-8 | UDAP registered as spoke. Analytics tools verified with correct regional scoping. Dynamic tool catalog reconciliation tested. Verify: AI query for settlement rates returns regionally scoped data, not empty |
...
| 8-9 | Write-back tested end-to-end: ADR closes a mediation case → `arc_close_mediation_case` → PrEPA records closure with benefits → UDAP picks up the closure via feed → data visible in analytics. Document upload tested: signed agreement → ECM |
...
Test multi-spoke AI sessions. These queries should route primarily through UDAP (which has the data), with write-backs going through ARC Integration API:
- "What are the triage outcomes for cases that became mediation sessions this quarter?" → UDAP (has both triage results and ADR outcomes ingested from ARC feed)
- "Show me settlement rates by region for Q1 FY2026" → UDAP `query_fct_adr_sessions` with OBO regional scoping
- "Close mediation case 370-2026-00123 as a successful settlement with $50k" → ARC Integration API `arc_close_mediation_case` → PrEPA → WAL/CDC → UDAP (data refreshes within seconds)
- "Prepare a litigation summary for charge 370-2026-00123" → UDAP for case history + ARC Integration API for real-time litigation detail from NVS
...
- [ ] Hub: OBO configuration for UDAP token delegation
